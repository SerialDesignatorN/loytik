...
namespace loytik {
	...
	loytik_module::loytik_module():
		_impl(std::make_unique<module_impl>())
	{
	}
	
	runtime_context* loytik_module::get_runtime_context() {
		return _impl->get_runtime_context();
	}
	
	void loytik_module::add_external_function_impl(std::string declaration, function f) {
		_impl->add_external_function_impl(std::move(declaration), std::move(f));
	}

	void loytik_module::add_public_function_declaration(std::string declaration, std::string name, std::shared_ptr<function> fptr) {
		_impl->add_public_function_declaration(std::move(declaration), std::move(name), std::move(fptr));
	}
	
	void loytik_module::load(const char* path) {
		_impl->load(path);
	}
	
	bool loytik_module::try_load(const char* path, std::ostream* err) noexcept{
		return _impl->try_load(path, err);
	}
	
	void loytik_module::reset_globals() {
		_impl->reset_globals();
	}
